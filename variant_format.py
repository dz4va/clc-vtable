...
class VariantFormat(object):
    ...
    def ProcessSNP(Ref, Allele, Crc):
        ...
        if Crc:
            ...
            coded = Crc.split(":c.")[1]
            if not VariantFormat.ContainsIllegal(coded, ["[", ";"]):
                # Extract the position
                final += VariantFormat.GetIntFromString(coded)
            else:
                # If it's not Valid raise Validation Exception
                fr = "{}{}{}{}".format(
                    final, ">", Allele, VariantFormat.ErrorValue(Crc))
    ...
    def ProcessInsertion(Ref, Allele, Crc):
        ...
        if Crc:
            ...
            if not VariantFormat.ContainsIllegal(coded, ["[", ";"]):
                # Extract the position
                final += VariantFormat.GetIntFromString(coded.split("_")[1])
            else:
                # If it's not Valid raise Validation Exception
                fr = "{}{}".format(
                    final, VariantFormat.ErrorValue(Crc))
    ...
    def CodonPos(fd):
        ...
        final = ""
        if fd:
            temp = VariantFormat.GetIntFromString(fd)
            if temp:
    ...
    def AminoAcidChange(aac):
        ...
        if aac:
            ...
            if not VariantFormat.ContainsIllegal(coded, ["[", ";"]):
                ...
                fr = "{}{}".format(
                    final, VariantFormat.ErrorValue(aac))
